 ...
 */
    ...
    if (type == 1) {
        a = 3432 * tb * kt * kbm * kvs * ko * km * kp; //Физик, ko нет, берем kvs по if выше
    } else {
        a = 2573 * tb * kt * kbm * kvs * ko * km * kp * kpr; //Юрик, kvs нет, берем ko = 1.8
    }
    
    var aa = 'Результат'
    if (type == 1) {
        aa = "3432" + " * " + kt + " * " + kbm + " * " + kvs + " * " + ko + " * " + km + " * " + kp + " * " + "1"; //Физик
    } else {
        aa = "2573" + " * " + kt + " * " + kbm + " * " + kvs + " * " + ko + " * " + km + " * " + kp + " * " + kpr; //Юрик
    }
 ...
 */
    ...
    if (km <= 1.1 || kp < 1 ) {
        ...
        nasko = "23%, Кроме: ВАЗ/LADA/ГАЗ/УАЗ, RENAULT LOGAN, DAEWOO NEXIA СТАРШЕ 3-х лет от года выпуска по ПТС";
    }
